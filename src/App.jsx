...
export default function CannonBlastGame() {
  ...
  useEffect(() => {
    const resizeCanvas = () => {
      ...
      if (canvas) {
        ...
        game.current.width = window.innerWidth;
        game.current.height = window.innerHeight;
      }
    };
  ...
  // --- GENERADOR PROCEDURAL MEJORADO (Modo Infinito) ---
  const generateProceduralLevel = (world, round, startX, floorY) => {
    const blockSize = 60;
    const blocks = [];
    
    // Probabilidad de reforzados según ronda
    let reinforcedProb = 0.25;
    if (round > 5) reinforcedProb = 0.40;
    if (round > 15) reinforcedProb = 0.60;

    // FACTOR DE ESCALA: Cada 7 rondas, aumentamos el tamaño base de las estructuras
    ...
    const createBlock = (x, y, type) => {
      ...
      return Matter.Bodies.rectangle(x, y, blockSize, blockSize, {
        ...
        restitution: 0.0, // Estabilidad total
        custom: { hp: isReinforced ? 3 : 1, maxHp: isReinforced ? 3 : 1, type }
      });
    };

    // Selección de Patrón (1-12)
    const patternId = ((round - 1) % 12) + 1; 

    // NOTA: floorY es la parte SUPERIOR de la plataforma.
    // Los bloques deben generarse hacia arriba: y - blockSize/2, y - blockSize*1.5, etc.

    switch(patternId) {
        case 1: // Torre Simple (Crece en altura)
            ...
            break;
        case 2: // Pirámide Aleatoria (Crece en capas)
            ...
            break;
        case 3: // Castillo (Torres crecen en altura)
            const towerH = 4 + scale;
            for(let i=0; i<towerH; i++){
                const y = floorY - blockSize/2 - (i * blockSize);
                blocks.push(createBlock(startX - blockSize*1.5, y, 'reinforced')); // Torre Izq
                blocks.push(createBlock(startX + blockSize*1.5, y, 'reinforced')); // Torre Der
            }
            // Muro central crece en altura tambien
             for(let i=0; i<2+scale; i++) {
                blocks.push(createBlock(startX, floorY - blockSize/2 - i*blockSize, 'wood'));
             }
            break;
        case 4: // Puente Colgante (Pilar crece en altura + mas cosas encima)
            const bridgeH = 1 + scale; // Altura del pilar
            for(let i=0; i<bridgeH; i++) {
                const y = floorY - blockSize/2 - i*blockSize;
                blocks.push(createBlock(startX - blockSize*2, y, 'reinforced'));
                blocks.push(createBlock(startX + blockSize*2, y, 'reinforced'));
            }
            // Tablón sobre los pilares
            ...
            // Cajas sobre el puente (aumentan con scale)
            ...
            break;
        case 5: // Pared Doble (Crece en altura)
            const wallH = 5 + scale;
            ...
            break;
        case 6: // L Lateral (Se hace más alta y ancha)
        ...
        case 7: // Zig Zag (Más alto)
             ...
             break;
        case 8: // Diamante (Se hace más alto en el centro)
             // Base 3
             ...
             break;
        case 9: // Estrella (+) (Centro más alto)
             blocks.push(createBlock(startX, floorY - blockSize/2, 'reinforced')); // Base
             const centerH = 1 + scale;
             for(let i=0; i<centerH; i++) {
                 const y = floorY - blockSize*1.5 - i*blockSize;
                 blocks.push(createBlock(startX, y, 'reinforced')); // Center column
                 // Brazos en la parte baja del centro
                 if (i===0) {
                     blocks.push(createBlock(startX - blockSize, y, 'wood')); 
                     blocks.push(createBlock(startX + blockSize, y, 'wood')); 
                 }
             }
             blocks.push(createBlock(startX, floorY - blockSize*(1.5+centerH), 'wood')); // Top
             break;
        case 10: // Arco Romano (Pilares más altos)
             const arcH = scale;
             ...
             break;
        case 11: // Trampa (Más altura para desestabilizar)
             blocks.push(createBlock(startX - blockSize/2, floorY - blockSize/2, 'wood'));
             blocks.push(createBlock(startX + blockSize/2, floorY - blockSize/2, 'wood'));
             // Soportes extra si scale alto
             for(let i=0; i<scale; i++) {
             ...
             break;
        default: // 12 - Fortaleza (Más ancha y alta)
             const fortH = 3 + Math.floor(scale/2);
             const fortW = 1 + Math.ceil(scale/2); // Expande hacia los lados
  ...
  const initLevel = useCallback((levelIdOrMode, roundNum = 1, currentAmmo = null) => {
    ...
    // Solo resetear score si es nivel 1 o primera ronda infinita
    ...
    if (gameMode === 'infinite') {
        ...
        // Lógica original de campaña (simplificada aquí para brevedad, usando la lógica existente)
        const levelData = LEVELS.find(l => l.id === levelIdOrMode);
        const blockSize = 60;
        ...
        // ... (Mantener lógica de patrones de campaña existente) ...
    ...
    if (game.current.bodiesToRemove.length > 0) {
      ...
      if (remainingTargets.length === 0) {
        ...
        setScore(s => {
            ...
            // Actualizar HighScore en Infinito
  ...
  const drawBackground = (ctx, w, h) => {
    // Fondo más dinámico para modo infinito
    if (gameMode === 'infinite') {
        const grad = ctx.createLinearGradient(0, 0, 0, h);
        grad.addColorStop(0, '#2b1055'); // Púrpura oscuro
        grad.addColorStop(1, '#7597de'); // Azul
        ctx.fillStyle = grad;
  ...
  // Reutilizamos las funciones de dibujo existentes sin cambios grandes...
  ...
  useEffect(() => {
    if (gameState === 'playing') {
      if (gameMode === 'campaign') {
         ...
         initLevel('infinite', round, round === 1 ? null : ammo); // Pasar ammo para calcular bonus
      }
      renderReqRef.current = requestAnimationFrame(renderLoop);
    } else {
  ...
  const MainMenu = () => (
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-gradient-to-br from-orange-100/90 via-sky-200/90 to-emerald-100/90 backdrop-blur-sm p-4">
      <div className="bg-white/80 p-8 md:p-12 rounded-3xl shadow-2xl text-center max-w-2xl border border-white/50 backdrop-blur-md animate-in zoom-in duration-500">
        <h1 className="text-6xl md:text-8xl font-black mb-4 bg-gradient-to-r from-orange-500 to-amber-600 bg-clip-text text-transparent drop-shadow-xl">
          CANNON BLAST
        </h1>
        ...
        >
          ...
          <span className="relative flex items-center justify-center gap-3 text-2xl md:text-3xl font-bold text-white tracking-wide">
            <Play fill="currentColor" size={32} /> JUGAR AHORA
          </span>
        </button>
      </div>
    </div>
  ...
  const LevelSelect = () => (
    <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md overflow-y-auto">
      <div className="min-h-full flex flex-col items-center justify-center p-8">
        <h2 className="text-5xl font-bold text-white mb-8 drop-shadow-lg text-center mt-8">Selecciona Modo</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl flex-shrink-0">
          ...
          <button
            ...
            className="col-span-1 md:col-span-3 group relative bg-gradient-to-r from-indigo-900 to-purple-900 border-4 border-purple-500/30 hover:border-purple-400 p-8 rounded-3xl transition-all hover:-translate-y-2 shadow-2xl overflow-hidden flex flex-col md:flex-row items-center gap-8"
          >
            <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-20"></div>
            <div className="w-32 h-32 rounded-full bg-purple-600/30 flex items-center justify-center animate-spin-slow shadow-[0_0_30px_rgba(168,85,247,0.5)]">
              <Infinity size={64} className="text-purple-200" />
            </div>
            <div className="flex-1 text-center md:text-left z-10">
               <h3 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-pink-200 mb-2">MODO INFINITO</h3>
               <p className="text-purple-200 text-lg mb-4">¡Supervivencia extrema! Balas sobrantes = Bonus</p>
               <div className="flex items-center justify-center md:justify-start gap-4">
                  <span className="bg-black/40 px-4 py-2 rounded-lg text-purple-300 flex items-center gap-2 border border-purple-500/30">
          ...
          {LEVELS.map((level) => (
            <button
              ...
              className="group relative bg-white/10 border-2 border-white/20 hover:border-emerald-400 p-8 rounded-3xl transition-all hover:-translate-y-2 hover:bg-white/20 flex flex-col items-center gap-4 overflow-hidden"
            >
              <div className={`w-20 h-20 rounded-2xl flex items-center justify-center text-4xl font-bold shadow-lg transform group-hover:scale-110 transition-transform ${
              ...
              <div className="text-center">
                ...
                <p className="text-gray-300 text-sm">Munición: {level.ammo}</p>
              </div>
            </button>
          ))}
        </div>
  ...
  const VictoryScreen = () => {
    // Si es modo infinito, mostramos la pantalla de ronda ganada
    if (gameMode === 'infinite') {
        return (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/90 backdrop-blur-lg animate-in zoom-in duration-300">
                <h2 className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 mb-4 drop-shadow-2xl">
                ...
                <div className="flex gap-4">
                     <button 
                        onClick={() => {
                            setRound(r => r + 1);
                            // IMPORTANTE: El ammo ya se actualizó en la lógica de victoria,
                            // pero necesitamos resetear el estado 'playing' para que initLevel se llame
                            setGameState('playing'); 
                        }}
                        className="px-10 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 rounded-xl text-white font-bold text-xl shadow-lg hover:shadow-purple-500/50 hover:scale-105 transition-all flex items-center gap-2"
                     >
    ...
    // Pantalla de Victoria Normal (Campaña)
    const level = LEVELS.find(l => l.id === currentLevelId);
    return (
      <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-lg animate-in fade-in duration-500">
        <div className="text-center transform transition-all scale-100 p-6">
          <h2 className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-yellow-600 mb-8 drop-shadow-2xl">
            ¡NIVEL COMPLETADO!
          </h2>
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-gradient-to-b from-sky-300 to-sky-100 select-none font-sans">
      ...
      {gameState === 'playing' && (
        <div className="absolute top-0 left-0 right-0 p-4 md:p-6 flex justify-between items-start pointer-events-none z-10">
          <div className="flex flex-col gap-2">
            ...
            {gameMode === 'infinite' ? (
                <div className="flex gap-4">
                     <div className="bg-purple-900/80 backdrop-blur-md text-white px-5 py-2 rounded-2xl border border-purple-500/50 shadow-lg animate-in slide-in-from-top-4">
                        <span className="text-purple-300 text-xs uppercase font-bold tracking-wider block">Ronda</span>
                        <div className="text-2xl font-black flex items-center gap-2">
                            <Infinity size={24} className="text-purple-400" /> {round}
                        </div>
                     </div>
                     <div className="bg-black/60 backdrop-blur-md text-white px-5 py-2 rounded-2xl border border-white/10 shadow-lg flex items-center gap-3 animate-in slide-in-from-left-4">
                        <span className="text-yellow-400"><Trophy size={18}/></span>
                ...
                /* HUD MODO CAMPAÑA */
                <>
                <div className="bg-black/60 backdrop-blur-md text-white px-5 py-2 rounded-2xl border border-white/10 shadow-lg animate-in slide-in-from-top-4">
                   <span className="text-gray-400 text-xs uppercase font-bold tracking-wider block">Objetivo</span>
                   <div className="text-xl font-black">{LEVELS.find(l => l.id === currentLevelId).name}</div>
                </div>
                <div className="bg-black/60 backdrop-blur-md text-white px-5 py-2 rounded-2xl border border-white/10 shadow-lg flex items-center gap-3 animate-in slide-in-from-left-4">
                   <span className="text-yellow-400"><Star fill="currentColor" size={18}/></span>
                   <div className="text-2xl font-mono font-bold">{score}</div>
                </div>
                </>
            )}
          ...
          <div className="flex flex-col items-end gap-2">
            <div className={`backdrop-blur-md p-3 rounded-2xl border shadow-lg animate-in slide-in-from-right-4 ${gameMode === 'infinite' ? 'bg-purple-900/60 border-purple-500/30' : 'bg-black/60 border-white/10'}`}>
              <span className={`text-xs uppercase font-bold tracking-wider block mb-1 text-right ${gameMode === 'infinite' ? 'text-purple-300' : 'text-gray-400'}`}>Munición</span>
              <div className="flex gap-2">
                {Array.from({ length: Math.max(0, ammo) }).map((_, i) => (
                  <div key={i} className={`w-6 h-6 md:w-8 md:h-8 rounded-full shadow-inner border animate-pulse ${gameMode === 'infinite' ? 'bg-gradient-to-br from-pink-400 to-purple-600 border-purple-800' : 'bg-gradient-to-br from-gray-300 to-gray-600 border-gray-800'}`}></div>
                ))}
                {ammo === 0 && <span className="text-red-400 font-bold px-2 self-center animate-pulse">¡VACÍO!</span>}
              </div>
            </div>
      ...
      {gameState === 'lost' && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md animate-in zoom-in">
           <h2 className="text-6xl font-black text-red-500 mb-4 drop-shadow-lg">GAME OVER</h2>
           {gameMode === 'infinite' ? (
               <div className="text-center mb-8">
                   <p className="text-white text-2xl mb-2">Has sobrevivido <span className="text-purple-400 font-bold">{round}</span> Rondas</p>
                   <p className="text-gray-400">Puntuación Final: {score}</p>
               </div>
           ) : (
               <p className="text-white text-xl mb-8 opacity-80">Te has quedado sin balas de cañón.</p>
           )}
